...
Tonic's output tells you *what each side did*:
...
An npm build of the same API ships as **`@mergetonic/core`** in [`packages/tonic-core/`](../packages/tonic-core/) (this repo) or from npm after publish. It includes unit tests ported from `tests/test_tonic.py` plus a small **Python parity** suite ([`parity_gate.py`](tests/parity_gate.py)) that compares merge state and annotated output to this package.

Install / test from the monorepo root:
...
The PyPI distribution exposes a `merge-tonic` console script (same subcommands as the TypeScript CLI):

```bash
pip install mergetonic
merge-tonic merge --left a.txt --right b.txt
merge-tonic m -l a.txt -r b.txt
merge-tonic r a.txt b.txt -p x.txt -o report.json
merge-tonic a conflicted-with-tonic-markers.txt
merge-tonic report --left a.txt --right b.txt --path x.txt --out report.json
merge-tonic git --repo . compare --left-ref HEAD --right-ref HEAD --dry-run
```

Editable monorepo install: `pip install -e "./merge-tonic-lib[dev]"` then `python -m tonic.cli ...`.
