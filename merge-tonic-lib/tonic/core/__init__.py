...
def initial_state(lines, commit_id=None):
    return serialize_state(
        [
            (line, i, False, 1, [commit_id] if commit_id else [])
...
def current_lines(raw_state):
    ...
    return [line for (line, depth, anchored_right, count, provenance) in state if count % 2]

# Called at commit time
# returns state_string
def update_state(raw_state, lines, commit_id=None):
    state = deserialize_state(raw_state)
    if not state:
        return initial_state(lines)
    current_visible_lines = [
        line for (line, depth, anchored_right, count, provenance) in state if count % 2
    ]
    ...
    for pos in range(len(state)+1):
        if pos_in_insertions < len(insertions) and insertions[pos_in_insertions][0] == pos:
            ...
            if up:
                result.append(
                    (newlines[0], state[pos-1][1]+1, False, 1, [commit_id] if commit_id else [])
                )
            else:
                result.append(
                    (newlines[0], state[pos][1]+1, True, 1, [commit_id] if commit_id else [])
                )
            for line in newlines[1:]:
                result.append(
                    (line, result[-1][1]+1, False, 1, [commit_id] if commit_id else [])
                )
            pos_in_insertions += 1
        if pos < len(state):
...
def merge_states(state1, state2):
    ...
    for i in range(len(status_lines)+1):
        if i == len(status_lines) or (status_lines[i][5] and
                status_lines[i][6] and status_lines[i][0].strip()):
            found_add = False
            hit_left = False
            hit_right = False
            for j in range(begin, i):
                line, depth, anchored_right, in_child, provenance, on_left, on_right = status_lines[j]
                if on_left != on_right:
            ...
            begin = i + 1
    return (serialize_state([x[:5] for x in status_lines]), show_conflicts(result_lines))
